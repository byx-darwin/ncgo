...
## 4. 第一阶段必须先做的基础任务（均已完成）

以下任务不直接对应某条 lint 规则，属于所有规则的前置依赖。T001 ~ T003 已全部落地，记录存档供参考。
...
## 5. Phase 1 规则实现任务拆分（均已完成）

以下任务直接对齐 `phase1` 规则。T101 ~ T301 已全部落地，以下内容记录存档供参考。
...
以下为 proto lint 功能从设计到落地的实际执行顺序，所有步骤均已完成：

1. `T001` 解析后端确定（`bufbuild/protocompile`，见 `internal/protolint/load.go`）
2. `T002` 统一模型（`internal/protolint/types.go`、`load.go`）
3. `T003` 规则注册表与 diagnostic 结构（`rules.go`、`diag.go`）
4. `T101` / `T102` / `T201`（`rule_reqresp.go`、`rule_dynamic_http.go`）
5. `T202` / `T203` / `T204` / `T205` / `T206`（`rule_dynamic_http.go`）
6. `T301`（`rule_dynamic_http.go`）
7. 解析层测试 + 规则单测 + 规则集集成测试（`rules_test.go`、`load_test.go`）
8. CLI / JSON / MCP 包装（`internal/cli/protolint.go`、`internal/mcp/tools.go`）
