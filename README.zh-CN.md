...
- 生命周期工具：metadata 升级与保守的 `extract domain --apply`。
...
如果要在 managed 文件内部（而不是 `AGENTS.local.md`）直接插入自定义内容，请用 `<!-- ncgo:custom:<name>:start -->` / `<!-- ncgo:custom:<name>:end -->` 包裹（`<name>` 需匹配 `^[a-z][a-z0-9-]{0,62}$`）。下次 `ai sync` 时，格式正确的锚点内容会被保留，并重新追加到 `## Custom Notes` 小节；锚点之外的内容每次 sync 仍会被覆盖，行为与之前一致。若锚点格式有误（缺少结束标记、名字重复、名字不合法），`ai sync` 会拒绝覆盖该文件——与缺失 `ncgo:managed` 标记时的行为一致——直到你修复锚点或传入 `--force`；`--force` 只会绕过这个"格式有误锚点"的拒绝逻辑，格式正确的锚点无论是否传 `--force` 都会被保留并合并回去。
...
`release_canary` 会生成 `internal/base/release/canary.go`，并按服务类型额外生成 `hertz.go` 或 `kitex.go`。当前实现是 SDK-neutral helper，支持 release metadata、traffic context、Hertz Header adapter、Kitex metadata adapter、统一 canary rule、Nacos/Polaris discovery instance 模型、`Discoverer` / `RuleProvider` / `Selector` 抽象、stable/canary pool 拆分、权重选择、`fallback=stable|fail_fast`、Kitex load balancer 以及 Nacos/Polaris discoverer / rule-provider skeleton；后续再接真实 SDK adapter。
...
`ncgo mcp serve` 会启动 stdio MCP server。完整注册工具面（输入、输出格式、稳定字段、安全提示、副作用以及 CLI↔MCP 能力矩阵）由实际 metadata 生成，见 [MCP 工具参考](docs/mcp-reference.zh-CN.md)，因此 README 不再维护第二份工具清单。MCP 工具会保留 `content[0].text` 作为展示/转存载荷，通过 `structuredContent` 提供带版本的 `ncgo.mcp.result/v1` 机器契约，并在迁移期继续保留旧同级字段。工具注解会声明只读、破坏性、幂等、网络、外部进程与 dry-run 行为；`output` 只影响文本载荷格式。
