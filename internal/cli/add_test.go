...
func TestRunAddRPCMethodJSONOutput(t *testing.T) {
	root := seedAddRPCMethodProject(t)
	var out bytes.Buffer
	cmd := &cobra.Command{}
	cmd.SetOut(&out)

	err := runAddRPCMethod(cmd, &addRPCMethodOptions{root: root, service: "demo", rpc: "Ping", output: "json"})
	if err != nil {
		t.Fatalf("runAddRPCMethod json: %v", err)
	}
	var got map[string]any
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("json unmarshal: %v\n%s", err, out.String())
...
func seedAddRPCMethodProject(t *testing.T) string {
	t.Helper()
	root := t.TempDir()
	m := &manifest.Manifest{
		...
		Service: manifest.Service{
			Name: "demo", Kind: manifest.KindKitex, IDL: "idl/app/demo.proto",
		},
