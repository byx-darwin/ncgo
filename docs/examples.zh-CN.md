...
生成的 Kitex 项目同样构建在 go-tools v0.3.0 之上：`go.mod` 声明 `go 1.26.5`，并固定 `go-common v0.3.0` + `go-framework v0.3.0` + `go-middleware v0.1.0`。RPC 错误经 `internal/pkg/rpcerror`，通过 `go-framework/kitex/rpcerror` 把 `goerror` 错误映射为 Kitex `BizStatusError`；框架码来自 `go-framework/error`（`CodeInternalError=CodeSystem=10000`、`CodeConfigInvalid=10004`、`CodeRPCTimeout=10011`、`CodePermissionDenied=CodeAuthFailed=10002`）。业务码须 `>= 40100`。conf 中的 duration 类字段统一使用 `config.Duration`，在 `conf/dev/conf.yaml` 中以 duration 字符串（`"3s"`、`"30s"` 等）填写。
